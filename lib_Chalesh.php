...
// INSERT INTO `CHALESH` (`id`, `username`, `password`, `Date`) VALUES (NULL, 'username', 'password', 'Date ')
public function insert($QU){
$qr = "INSERT INTO `".CHALESH."` (`id`, `QU`) VALUES (NULL, '$QU');";
$result = $this->query($qr);
return $result;
}

public function select($id){
$qr = "SELECT `QU` FROM `".CHALESH."` WHERE id = '$id'";
$result = $this->query($qr);
...
$qr = "DELETE FROM `".CHALESH."` WHERE id = '$id'";
$result = $this->query($qr);
$row = mysqli_fetch_array($result);
return $row;
}    
...
$qr = "UPDATE `".CHALESH."` SET `QU`='$QU' WHERE `id`='$id'";
$result = $this->query($qr);
$row = mysqli_fetch_array($result);
return $row;
}
public function updatepassword_username($username){
$qr = "UPDATE `".CHALESH."` SET `password` = '$password' WHERE  `username`='$username'";
$result = $this->query($qr);
return $result;
}

public function count(){
$qr = "select * from ".CHALESH;
$result = $this->query($qr);
