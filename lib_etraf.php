...
class lib_etraf{
    ...
    // INSERT INTO `ETRAF` (`id`, `username`, `password`, `Date`) VALUES (NULL, 'username', 'password', 'Date ')
    public function insert($QU){
        $qr = "INSERT INTO `".ETRAF."` (`id`, `QU`) VALUES (NULL, '$QU');";
        $result = $this->query($qr);
        return $result;
    }

    public function select($id){
        $qr = "SELECT `QU` FROM `".ETRAF."` WHERE id = '$id'";
        $result = $this->query($qr);
    ...
    public function delete($id){
        $qr = "DELETE FROM `".ETRAF."` WHERE id = '$id'";
        $result = $this->query($qr);
        $row = mysqli_fetch_array($result);
        return $row;
    }    
    ...
    public function update($id,$QU){
        $qr = "UPDATE `".ETRAF."` SET `QU`='$QU' WHERE `id`='$id'";
        $result = $this->query($qr);
        $row = mysqli_fetch_array($result);
        return $row;
    }
    public function count(){
        $qr = "select * from ".ETRAF;
        $result = $this->query($qr);
